...
class Strategy_SRSS_Employee(Strategy):
	network = None
	controlNet = None
	...
	# send the local collision queue to the manger and receive the next step moving coodination
	def get_moving_info(self, collision_queue):
...
class Strategy_SRSS_Manager(Strategy):
	network = None
	controlNet = None
	send_data_history = None
	time_start = None 					# For task release simulation -> Future work: real dynamic change.
...
if __name__ == '__main__':
	...
	simulate_num_robots_manager = 5
	simulate_num_robots_employee = 15
	...
	if int(index) <= 5:
		strategy_SRSS_Manager = Strategy_SRSS_Manager(id=int(index), \
									coordinate=coordinate, \
									direction=[1, 3], \
									step_size=10, \
									robot_radius=10, \
									go_interval=0.1, \
									num_robots=simulate_num_robots_manager, \
									# energy_level=energy_level[11][int(index) - 1], \
									energy_level=energy_level[0][int(index) - 1], \
									controlNet='172.16.0.254')

		strategy_SRSS_Manager.global_task_list = [{'start': 5, 'duration': 40, 'radius' : 200, 'coordinate': [325, 475], 'new_task': True}]

		while True:
			strategy_SRSS_Manager.go()		

	else:
		strategy_SRSS_Employee = Strategy_SRSS_Manager(id=int(index), \
								coordinate=coordinate, \
								direction=[1, 3], \
								step_size=10, \
								robot_radius=10, \
								go_interval=0.1, \
								num_robots=simulate_num_robots_employee, \
								# energy_level=energy_level[11][int(index) - 1], \
								energy_level=energy_level[0][int(index) - 1], \
								controlNet='172.16.0.254')

		strategy_SRSS_Employee.global_task_list = [{'start': 5, 'duration': 10, 'radius' : 100, 'coordinate': [1200, 700], 'new_task': True},
													{'start': 5, 'duration': 10, 'radius' : 70, 'coordinate': [1075, 175], 'new_task': True}]

		while True:
			strategy_SRSS_Employee.go()
	...
	# while True:
	# 	# strategy_SRSS_Manager.go()
	# 	strategy_SRSS_Manager_thread = threading.Thread(target = strategy_SRSS_Manager.go)
	# 	strategy_SRSS_Employee_thread = threading.Thread(target = strategy_SRSS_Employee.go)
		
	# 	strategy_SRSS_Manager_thread.start()
	# 	strategy_SRSS_Employee_thread.start()
